...
for(m in 1:repetition){
  ...
  sim_base_control=sim_base_control %>% mutate(dropout_time = runif(n(), min=0, max=1)*25,
  ...
  sim_base_intensive = sim_base_intensive %>% mutate(dropout_time = runif(n(), min=0, max=1)*25,
                                                     dropout_time = round(dropout_time, 1)) %>%
    mutate(i_dropout_time = ifelse(dropout_time>5, 0, 1))
...
write.csv(result_df, file=paste0("7_10000_summary/7_10000_" , array_index , ".csv"))
write.csv(result_summary, file=paste0("7_10000_eventsummary/7_10000_event_" , array_index , ".csv"))
